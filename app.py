...
def compute_descriptors(df, smiles_col):
    """
    Calculate molecular descriptors once and cache the serializable result.
    RDKit molecule objects are intentionally kept out of the cached result.
    """
    result = df.copy()
    ...
    for column, default in descriptor_defaults.items():
        if column == "ValidSMILES":
            result[column] = False
        elif column not in result.columns:
            result[column] = default

    for idx, smiles in result[smiles_col].items():
        mol = mol_from_smiles(smiles)

        if mol is None:
            continue

        result.at[idx, "ValidSMILES"] = True
        result.at[idx, "MolWt"] = Descriptors.MolWt(mol)
        result.at[idx, "LogP"] = Crippen.MolLogP(mol)
        result.at[idx, "NumHDonors"] = Lipinski.NumHDonors(mol)
        result.at[idx, "NumHAcceptors"] = Lipinski.NumHAcceptors(mol)
        result.at[idx, "TPSA"] = rdMolDescriptors.CalcTPSA(mol)
        result.at[idx, "NumRotatableBonds"] = Lipinski.NumRotatableBonds(mol)
        result.at[idx, "NumAromaticRings"] = rdMolDescriptors.CalcNumAromaticRings(mol)

        violations = 0
        if result.at[idx, "MolWt"] > 500:
            violations += 1
        if result.at[idx, "LogP"] > 5:
            violations += 1
        if result.at[idx, "NumHDonors"] > 5:
            violations += 1
        if result.at[idx, "NumHAcceptors"] > 10:
            violations += 1

        result.at[idx, "LipinskiViolations"] = violations

    result["LipinskiDruglike"] = result["LipinskiViolations"].eq(0)

    return result


@st.cache_resource(show_spinner=False)
def build_molecule_objects(smiles_series):
    """Build RDKit molecule objects once and reuse them across reruns."""
    return tuple(mol_from_smiles(smiles) for smiles in smiles_series.tolist())
...
with st.spinner(
    ...
    df["mol_obj"] = list(
        build_molecule_objects(df[SMILES_COL])
    )
